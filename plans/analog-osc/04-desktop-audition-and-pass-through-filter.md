...
**Order:** 04 · **Depends on:** plans 01–03 · **State:** `[~]` raw audition,
shared selector, and baseline guards complete; `[ ]` event export and further
candidate adapters remain.

## DSP background

An oscillator heard through a filter, envelope, effects, and output processing
is not the oscillator alone. `PassThrough` is a filter choice whose output is
exactly its input, with no delay or stored tail. It reveals the pre-filter
source mix while intentionally leaving the later VCA and output path intact.
This plan makes visual improvements testable as musical instruments, not just
as oscilloscope traces.
...
- Oscillator Lab research UI: plans/analog-osc/05-oscillator-lab.md
- Common comparison protocol:
  plans/analog-osc/03-evaluation-and-hardware-selection.md
