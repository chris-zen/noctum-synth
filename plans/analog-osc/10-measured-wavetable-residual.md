...
**Order:** 10 · **Depends on:** plans 01–07 · **State:** `[x]` frozen as a
...
Pitch-conditioned tables capture changes across the keyboard. The “residual”
alternative stores only the difference between the production oscillator and
the target; it is useful only if that difference compresses better or behaves
better during transitions. Complex-spectrum interpolation blends harmonic
amplitude and phase, while mip selection removes harmonics unsafe at the output
sample rate.
...
At runtime, interpolation is two-dimensional: target character varies between
pitch knots, while anti-alias bandwidth varies between mip levels.

```rust
let lo = sample(table[pitch_lo][mip], phase);
let hi = sample(table[pitch_hi][mip], phase);
let pitch_blend = lerp(lo, hi, pitch_fraction);
```

Plan 11 extends this with a second blend between adjacent safe mips. Neither
blend may select a table containing harmonics above the current safety limit.
...
Cycle-to-cycle drift and variation are added through plan 15. Optional
