...
**Order:** 03 · **Depends on:** plan 01 · **State:** `[~]` metric revision 1
and compact static/dynamic gates complete; `[ ]` full matrix and hardware
aggregation remain.

## DSP background

No single number measures oscillator quality. Time-domain error shows waveform
shape; a spectrum shows energy by frequency; harmonic error measures components
that belong to the note; residual energy finds components that do not. A
high-rate render filtered down to the test rate is a useful implementation
reference, but it is not automatically ground truth for an analog target.

Listening is level-matched because louder signals are commonly preferred even
when their timbre is not closer. ABX tests audibility; target matching tests
fidelity. CPU percent, latency, state bytes, and immutable table bytes are
reported independently so a later hardware choice does not distort the sound
research.
...
- Plan 09 adds a reproducible static log-pitch scheduler at 48/96 kHz. It
...
- Plan 09 also provides the first fixed-seed, RMS-matched named/ABX/target-match
  listening package with hashed manifests, a separate answer key, and a
  response template. Its recorded results are summarized in plan 09.
...
Plan 10 now contributes the first compact dynamic extension without changing
...
For two equal-length signals, normalized RMS error can be introduced as:

```text
error = sqrt(mean((candidate - reference)^2))
NRMSE = error / max(RMS(reference), small_epsilon)
```

The denominator prevents a quiet reference from making the raw error look
artificially small. Reports must still preserve unnormalized levels because
normalization can hide a real gain mismatch.
