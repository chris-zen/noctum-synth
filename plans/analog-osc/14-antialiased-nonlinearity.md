...
**Order:** 14 · **Depends on:** a retained deterministic parent from plans
09–13 · **State:** `[ ]` planned.

## DSP background

A nonlinear function such as soft clipping creates new harmonics. When those
harmonics exceed Nyquist they alias, even if the incoming oscillator was
perfectly bandlimited. Antiderivative antialiasing (ADAA) evaluates integrated
forms of the nonlinearity so the discrete result produces fewer aliases.
Oversampling is the comparison reference: process at a higher rate, low-pass,
then return to the normal rate. The stage is justified only by measured target
behavior, not by adding generic saturation for loudness.
...
Use level-sweep captures from plan 06. A nonlinear stage is justified only when:
...
First-order ADAA replaces direct evaluation with a divided difference of an
antiderivative `F` of the nonlinear function `f`:

```rust
let dx = x - previous_x;
let y = if dx.abs() > epsilon {
    (antiderivative(x) - antiderivative(previous_x)) / dx
} else {
    nonlinearity(0.5 * (x + previous_x))
};
previous_x = x;
```

The near-zero branch avoids unstable division. The exact antiderivative and
latency/alignment rules must be derived for each candidate nonlinearity.
