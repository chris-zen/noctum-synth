...
**Order:** 08 · **Depends on:** plans 06–07 · **State:** `[ ]` planned; the
static oscillator protocol used as its host proof is complete.

## DSP background

A synthesizer voice is more than its oscillator. Filters change spectrum and
phase, envelopes change level over time, control laws map MIDI values to
physical parameters, and modulation paths determine response to LFOs and other
sources. Characterising these separately makes later models identifiable: it
prevents filter coloration, converter coupling, or an envelope transient from
being mistaken for oscillator character.
...
The host remains [`07-automated-synth-reference-capture.md`](07-automated-synth-reference-capture.md).
...
| `oscillator-static-v1` | Saw / triangle / 50% pulse × pitch | Plan 07 matrix (MIDI 16–90, osc source per target) | 226 | 30–40 min | Pitch ±50¢; non-silent; spectral distinctness | Cycles, harmonics, roles (plan 07) | Host + osc doctor |
...
| `drift-longform-v1` | Slow drift, jitter, amplitude variation | Multi-minute held notes (plan 15) | 20–60+ min |
...
1. **Plan 07 host proven** — Arturia oscillator-static acceptance and Phase 6
   extraction, recorded in the status section of
   [`07-automated-synth-reference-capture.md`](07-automated-synth-reference-capture.md).
...
4. **Extractors for Tier A** — at minimum:
   - oscillator-static NPZ/JSON (plan 07),
...
| --- | --- |
| `07-automated-synth-reference-capture.md` | Host + first protocol (`oscillator-static-v1`). |
| `07-automated-synth-reference-capture.md` | Host implementation phases; characterisation follows its accepted static run. |
| `06-reference-capture-and-identification.md` (WIP tree) | Oscillator capture matrix inspiration; fitting/identification stay there. |
| `15-drift-variation-and-calibration.md` (WIP tree) | Consumes `drift-longform-v1` recordings; fitting not redefined here. |
| Oscillator candidate plans 09–17 | Consume derived oscillator/filter features; not capture definitions. |
...
- Host plan: `plans/analog-osc/07-automated-synth-reference-capture.md`
- Host execution phases: `plans/analog-osc/07-automated-synth-reference-capture.md`
- Oscillator research programme (WIP): `plans/analog-osc/00-master-plan.md`
- Capture/identification (WIP): `plans/analog-osc/06-reference-capture-and-identification.md`
- Drift/calibration (WIP): `plans/analog-osc/15-drift-variation-and-calibration.md`
