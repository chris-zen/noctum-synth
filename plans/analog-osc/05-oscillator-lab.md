...
**Order:** 05 · **Depends on:** plans 01–04 · **State:** `[~]` independent
model selection and rendering complete; `[ ]` A/B overlays, reference metrics,
diagnostics, and unified export remain.

## DSP background

An oscilloscope plots amplitude over time; an FFT plots how much energy appears
at each frequency. Neither view alone proves that a model sounds closer. The
lab therefore keeps waveform, spectrum, objective metrics, and listening
exports tied to the same exact render request. Its selector is deliberately
separate from the live synth selector so visual experiments cannot silently
...
Model B, target overlays, diagnostics, metrics, and export remain later Plan 05
