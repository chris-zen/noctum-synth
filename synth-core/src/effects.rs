...
impl EffectsState {
    ...
    pub fn next(
        ...
        crate::profiler_begin!(ctx, RenderStage::EffectsPreparation);
        let params = self.selected_params();
...
impl<K: ReverbKernel> Reverb<K> {
    fn next(
        ...
        crate::profiler_begin!(ctx, RenderStage::ReverbCombs);
        for index in 0..4 {
        ...
        crate::profiler_end!(ctx, RenderStage::ReverbCombs);
        let mut wet_left = sum_left * 0.18;
        let mut wet_right = sum_right * 0.18;
        crate::profiler_begin!(ctx, RenderStage::ReverbAllpasses);
        for index in 0..3 {
        ...
        crate::profiler_end!(ctx, RenderStage::ReverbAllpasses);
...
mod tests {
    ...
    fn tempo_is_initialized_and_clamped() {
        ...
        assert_eq!(effects.tempo_bpm(), DEFAULT_TEMPO_BPM);
    ...
    fn render_silence_peak(effect: &mut TestEffects, frames: usize) -> f32 {
        ...
        let mut ctx = crate::create_render_context!();
        for _ in 0..frames {
            let (left, right) = effect.next(0.0, 0.0, EffectModulation::default(), None, &mut ctx);
    ...
    fn delay_time_changes_do_not_jump_to_the_new_read_head_immediately() {
        ...
        let next = effect
            .next(0.8, 0.8, EffectModulation::default(), None, &mut ctx)
            .0;
    ...
    fn small_delay_memory_clamps_delay_time_and_remains_bounded() {
        ...
        for frame in 0..128 {
            ...
            let (left, right) =
                effects.next(input, -input, EffectModulation::default(), None, &mut ctx);
            peak = peak.max(left.abs()).max(right.abs());
        }
    ...
    fn undersized_buffered_effects_bypass_without_panicking() {
        ...
        for effect_type in [
            ...
            assert_eq!(
                effects.next(0.25, -0.5, EffectModulation::default(), None, &mut ctx),
                (0.25, -0.5)
            );
        }
    }
