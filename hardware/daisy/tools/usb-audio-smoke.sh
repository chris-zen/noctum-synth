...
audio_device=${USB_AUDIO_DEVICE:-Noctum (development)}
midi_device=${USB_MIDI_DEVICE:-Noctum USB MIDI (development)}
output=${USB_AUDIO_OUTPUT:-/tmp/noctum-usb-audio-smoke.wav}
...
if ! awk -v peak="$maximum_amplitude" 'BEGIN { exit !(peak > 0.00001) }'; then
    privacy_probe=/tmp/noctum-coreaudio-privacy-probe.wav
    rm -f "$privacy_probe"
