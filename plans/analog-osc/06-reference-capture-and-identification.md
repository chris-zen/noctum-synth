...
**Order:** 06 · **Depends on:** plans 01 and 03 · **State:** `[~]` Monologue
import and Prophet-5 V static reference complete; `[ ]` dynamic and physical
hardware datasets remain.

## DSP background

A single displayed cycle is not a reliable model: its level, phase origin,
pitch, and signal chain may be unknown. A target dataset records those facts and
keeps training notes separate from validation and blind test notes. A median
cycle represents the repeatable waveform; differences between cycles describe
noise, drift, and other stochastic behavior. Complex harmonics retain both the
strength and timing of each harmonic.
...
- The Prophet-5 V `oscillator-static-v1` run is accepted separately: 226
  verified cases and three extracted 75-note waveform datasets. It is a
  software-model reference, never labelled Prophet hardware.
...
The accepted static dataset is the first separate software-reference dataset.
Extend it through the same virtual routing when the following dynamic cases are
scheduled:
...
`plans/analog-osc/07-automated-synth-reference-capture.md`. For Prophet-5 V the
