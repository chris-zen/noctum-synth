...
pub(crate) fn draw_spectrum(
    ...
    let plot_rect = match state.source {
        SignalSource::Input => spectrum::render_spectra(ui, &[input_trace], &config, 0.0),
        SignalSource::Output => spectrum::render_spectra(ui, &[output_trace], &config, 0.0),
        SignalSource::InputAndOutput => {
            spectrum::render_spectra(ui, &[output_trace, input_trace], &config, 0.0)
        }
    };
    ...
    if let Some((freq, input_db, output_db, _, _)) = hover_info {
        ...
        *hover = Some(HoverStatus::Spectrum {
            freq_hz: freq,
            levels,
            note: format_midi_note(freq),
        });
    }
}
...
#[cfg(test)]
mod tests {
    use super::{SpectrumChannel, copy_channel};
