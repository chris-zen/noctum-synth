...
**Order:** 07 · **Depends on:** plan 06 · **State:** `[~]` capture, extraction,
and first live acceptance complete; final supply-chain/documentation work
remains.

## Status and execution phases

The host and first live software-reference run are usable. This checklist is
the preserved execution record; the detailed design below remains the source
of truth for behavior.

- [x] Phase 1 — crate scaffold, domain types, and protocol matrix.
- [x] Phase 2 — project persistence and read-only CLI.
- [x] Phase 3 — MIDI transport and Prophet-5 V adapter.
- [x] Phase 4 — audio capture, validation, resumable runner, and simulated E2E.
- [x] Phase 5 — doctor command, full CLI, and bounded terminal UX.
- [x] Phase 6 — Rust extraction, numerical parity, and live acceptance.
- [~] Phase 7 — supply-chain finish and documentation are in progress; live
  acceptance itself has passed.

The accepted run contains 226 complete and verified cases: one silence take
plus 75 notes for each of saw, triangle, and 50-percent pulse. Extraction
produced three datasets of 75 notes. Representative pitch error stayed inside
the ±50-cent gate, with maximum absolute error about 11 cents in the smoke set.
The accepted adapter is revision 5.

An earlier revision-4 milestone deliberately removed Fine Tune, Pulse Width,
and Filter Envelope Amount from the MIDI map and moved them to operator setup.
That decision is retained in revision 5. The live session also proved that a
lost MIDI mapping can corrupt a run; a fresh mapping and new project recovered
cleanly. The detailed acceptance evidence is in
`synth-capture/docs/prophet5-v1-acceptance.md`.

After every future phase or protocol extension:

1. Run its stated review gate.
2. Perform a defect-first review of the diff.
3. Fix all high- and medium-priority findings, plus safe inexpensive minor
   findings.
4. Re-run the gate before beginning the next phase.

## DSP and measurement background

MIDI tells a synthesizer which note and settings to use; an audio input records
the result. A capture **case** is one fully specified combination of target,
waveform, note, and settings. Extraction estimates the real fundamental pitch,
finds repeated periods, rejects abnormal cycles, computes a robust median
cycle, and transforms that cycle into complex harmonics. “Complex” here means
each harmonic keeps both amplitude and phase, which is necessary to reconstruct
edge position and ramp curvature rather than only spectral brightness.
