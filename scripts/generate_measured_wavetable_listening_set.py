#!/usr/bin/env python3
"""Generate the blind listening gate for the measured-wavetable runtime."""
...
MODELS = {
    ...
    "candidate": "korg-monologue-measured-wavetable-v1",
}

# These pitch indices appeared in either Plan 04 blind package. Some are
# validation rows rather than eligible test rows; keeping the complete set here
# makes the no-repeat policy explicit and independently testable.
PREVIOUSLY_HEARD_PITCH_INDICES = frozenset({1, 3, 35, 37, 67, 71})


def selected_fresh_test_rows(waveform_profile: dict) -> list[tuple[str, dict]]:
    available = [
        row
        for row in waveform_profile["cases"]
        if row["split"] == "test"
        and row["pitch_index"] not in PREVIOUSLY_HEARD_PITCH_INDICES
    ]
    if len(available) < 3:
...
def main() -> int:
    ...
    if profile["experiment_id"] != MODELS["candidate"]:
        raise ValueError(
            f"expected profile for {MODELS['candidate']}, "
            f"got {profile['experiment_id']}"
        )
    ...
    for waveform in WAVEFORMS:
        ...
        for register, row in selected_fresh_test_rows(profile["waveforms"][waveform]):
    ...
    manifest = {
        "schema_version": 1,
        "purpose": "fresh blind acceptance gate for Plan 07 measured wavetable",
        "model_id": profile["experiment_id"],
        ...
        "selection": {
            "split": "test only (never used to construct the measured bank)",
            "registers": "lowest, middle, and highest available fresh test row",
            "excluded_previously_heard_pitch_indices": sorted(
                PREVIOUSLY_HEARD_PITCH_INDICES
            ),
            "table_or_guard_tuning_after_reveal_forbidden": True,
        },
        "level_match": {
    ...
    (args.output_root / "README.md").write_text(
        ...
        "Do not open `named` or `answer-key.json` yet. First complete "
        "`blind-abx`, recording whether X equals A or B. Then complete "
        "`blind-target-match`, recording whether choice A or B is closer to "
        "the reference. Save all answers and confidence values (0 to 1) in "
